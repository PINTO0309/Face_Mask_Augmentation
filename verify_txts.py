...
parser.add_argument('--dim', type=str, default='640x480', help='input width and height, e.g. 640x480')
args = parser.parse_args()

if random.random() < 0.5:
    print('Verifying test.txt')
    jpgs_path = Path(f'data/300wlp-{args.dim}_masked/test.txt')
else:
    print('Verifying train.txt')
    jpgs_path = Path(f'data/300wlp-{args.dim}_masked/train.txt')

with open(jpgs_path.as_posix(), 'r') as f:
    jpg_names = [l.strip() for l in f.readlines()]
