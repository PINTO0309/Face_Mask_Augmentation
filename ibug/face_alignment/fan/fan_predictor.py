...
class FANPredictor(object):
    ...
    def __call__(self, image: np.ndarray, face_boxes: np.ndarray, rgb: bool = True,
        ...
        if face_boxes.size > 0:
            ...
            # landmarks, landmark_scores = self.net(face_patches)
            ...
            if return_features:
                return landmarks, landmark_scores, torch.cat((stem_feats, torch.cat(hg_feats, dim=1) * torch.sum(heatmaps, dim=1, keepdim=True)), dim=1)
            else:
