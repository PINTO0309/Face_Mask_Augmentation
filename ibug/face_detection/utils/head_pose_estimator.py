...
class HeadPoseEstimator(object):
    def __init__(self, mean_shape_path: str = os.path.join(os.path.dirname(__file__), 'data', 'bfm_lms.npy')) -> None:
    ...
    def __call__(
        ...
        if camera_matrix is None:
            if image_width <= 0 or image_height <= 0:
                ...
                camera_matrix = np.array([
                    [image_width + image_height, 0, image_width / 2.0],
        ...
        _, rvec, _ = cv2.solvePnP(
            self._mean_shape_5pts,
            np.expand_dims(landmarks, axis=1),
            camera_matrix,
            dist_coeffs,
