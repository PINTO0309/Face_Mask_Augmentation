...
class RetinaFacePredictor(object):
    def __init__(self, threshold: float = 0.8, device: Union[str, torch.device] = 'cuda:0', model: Optional[SimpleNamespace] = None, config: Optional[SimpleNamespace] = None) -> None:
        ...
        if 'state_dict' in pretrained_dict.keys():
            pretrained_dict = {key.split('module.', 1)[-1] if key.startswith('module.') else key: value for key, value in pretrained_dict['state_dict'].items()}
        else:
            pretrained_dict = {key.split('module.', 1)[-1] if key.startswith('module.') else key: value for key, value in pretrained_dict.items()}
    ...
    def get_model(name: str = 'resnet50') -> SimpleNamespace:
        ...
        if name == 'resnet50':
            return SimpleNamespace(weights=os.path.realpath(os.path.join(os.path.dirname(__file__), 'weights', 'Resnet50_Final.pth')),  config=SimpleNamespace(**deepcopy(cfg_re50)))
        elif name == 'mobilenet0.25':
            return SimpleNamespace(weights=os.path.realpath(os.path.join(os.path.dirname(__file__), 'weights', 'mobilenet0.25_Final.pth')), config=SimpleNamespace(**deepcopy(cfg_mnet)))
        else:
            raise ValueError('name must be set to either resnet50 or mobilenet0.25')

    @staticmethod
    def create_config(top_k: int = 750, conf_thresh: float = 0.02, nms_thresh: float = 0.4, nms_top_k: int = 5000) -> SimpleNamespace:
